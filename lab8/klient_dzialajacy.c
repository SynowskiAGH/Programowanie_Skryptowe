...
void handleMulticast(int udpSocket) {
    ...
    if (bind(udpSocket, (struct sockaddr*) &multicastAddr, sizeof(multicastAddr)) < 0) {
        perror("bind");
        exit(EXIT_FAILURE);
    }
    ...
    if (setsockopt(udpSocket, IPPROTO_IP, IP_ADD_MEMBERSHIP, (char *)&multicastRequest, sizeof(multicastRequest)) < 0) {
